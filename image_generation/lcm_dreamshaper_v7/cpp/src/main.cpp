...
int32_t main(int32_t argc, char* argv[]) try {
    ...
    OPENVINO_ASSERT(!read_np_latent || (read_np_latent && (num_images_per_prompt == 1)),
        "\"readNPLatent\" option is only supported for one output image. Number of image output was set to: " + std::to_string(num_images_per_prompt));
